...
if (!workflowId)
    throw new Error('No workflow ID in URL');
const API = '/api/workflows';
// DOM elements
const nameDisplay = document.querySelector('#workflow-name-display');
const editorName = document.querySelector('#wf-editor-name');
const editorMeta = document.querySelector('#wf-editor-meta');
const stepsList = document.querySelector('#steps-list');
const stepDetail = document.querySelector('#step-detail');
const stepDetailTitle = document.querySelector('#step-detail-title');
const stepLabel = document.querySelector('#step-label');
const stepParams = document.querySelector('#step-params');
const stepUpdate = document.querySelector('#step-update');
const stepDelete = document.querySelector('#step-delete');
const saveBtn = document.querySelector('#save-workflow');
const replayBtn = document.querySelector('#replay-workflow');
const replayStatus = document.querySelector('#replay-status');
const addStepBtn = document.querySelector('#add-step');
const addStepDialog = document.querySelector('#add-step-dialog');
const stepTypeSelect = document.querySelector('#step-type-select');
const addStepParams = document.querySelector('#add-step-params');
const addStepSubmit = document.querySelector('#add-step-submit');
const recordToggle = document.querySelector('#record-toggle');
const recordToggleContainer = document.querySelector('#record-toggle-container');
const screenImg = document.querySelector('#wf-screen');
const streamLabel = document.querySelector('#wf-stream-label');
const deviceSelect = document.querySelector('#wf-device-select');
let workflow = null;
let selectedStepId = null;
// Track which step type fields to show in detail
let detailStepType = null;
...
function renderSteps() {
    ...
    stepsList.replaceChildren(...items.map((step) => {
        ...
        if (step.stepOrder === items.length)
...
function renderStepTypeFields(stepType) {
    switch (stepType) {
        case 'tap':
            return `
...
async function addStep(stepType) {
    if (!workflowId)
        return;
    const label = (document.querySelector('#add-step-label'))?.value?.trim() ?? '';
    const body = { stepType, label: label || undefined };
    // Collect fields based on type
    const x = (document.querySelector('#as-x'))?.value;
    const y = (document.querySelector('#as-y'))?.value;
    const endX = (document.querySelector('#as-endX'))?.value;
    const endY = (document.querySelector('#as-endY'))?.value;
    const durationMs = (document.querySelector('#as-durationMs'))?.value;
    const waitMs = (document.querySelector('#as-waitMs'))?.value;
    const aiQuestion = (document.querySelector('#as-aiQuestion'))?.value;
    ...
    if (y)
        body.y = Number(y);
    if (endX)
        body.endX = Number(endX);
    if (endY)
        body.endY = Number(endY);
    if (durationMs)
        body.durationMs = Number(durationMs);
    if (waitMs)
        body.waitMs = Number(waitMs);
    if (aiQuestion)
...
async function updateSelectedStep() {
    if (!selectedStepId || !detailStepType)
        return;
    const body = {};
    const label = stepLabel.value.trim();
    if (label)
        body.label = label;
    const x = (document.querySelector('#dt-x'))?.value;
    const y = (document.querySelector('#dt-y'))?.value;
    const endX = (document.querySelector('#dt-endX'))?.value;
    const endY = (document.querySelector('#dt-endY'))?.value;
    const durationMs = (document.querySelector('#dt-durationMs'))?.value;
    const waitMs = (document.querySelector('#dt-waitMs'))?.value;
    const aiQuestion = (document.querySelector('#dt-aiQuestion'))?.value;
    ...
    if (y !== null && y !== undefined && y !== '')
        body.y = Number(y);
    if (endX !== null && endX !== undefined && endX !== '')
        body.endX = Number(endX);
    if (endY !== null && endY !== undefined && endY !== '')
        body.endY = Number(endY);
    if (durationMs !== null && durationMs !== undefined && durationMs !== '')
        body.durationMs = Number(durationMs);
    if (waitMs !== null && waitMs !== undefined && waitMs !== '')
        body.waitMs = Number(waitMs);
    if (aiQuestion !== null && aiQuestion !== undefined && aiQuestion !== '')
        body.aiQuestion = aiQuestion;
    if (url !== null && url !== undefined && url !== '')
        body.url = url;
    if (appBundleId !== null && appBundleId !== undefined && appBundleId !== '')
        body.appBundleId = appBundleId;
...
async function reloadWorkflow() {
    workflow = await request(`${API}/${workflowId}`);
    updateHeader();
    renderSteps();
}
...
async function startReplay() {
    ...
    try {
        const result = await request(`${API}/${workflowId}/replay`, { method: 'POST' });
...
function pollReplay() {
    ...
    const interval = setInterval(async () => {
        try {
            const status = await request(`/api/workflows/replay/${replayRunId}`);
...
async function updateDeviceAssignment(udid) {
    await request(`${API}/${workflowId}`, {
        method: 'PATCH',
        headers: { 'content-type': 'application/json' },
...
screenImg.addEventListener('pointerdown', (e) => {
    if (!recordToggle.checked || !workflow?.deviceUdid)
        return;
    e.preventDefault();
    // Tap at the point
...
document.querySelectorAll('.remote-button[data-dir]').forEach((btn) => {
    btn.addEventListener('click', async () => {
        ...
        let action = { type: 'swipe', startX: cx, startY: cy, endX: cx, endY: cy, durationMs: 300 };
        switch (dir) {
            case 'up':
...
addStepBtn.addEventListener('click', () => {
    // Reset form
    const labelField = document.querySelector('#add-step-label');
    labelField.value = '';
