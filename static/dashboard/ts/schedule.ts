...
interface ApiDevice {
    ...
    pluginData?: Record<string, { accounts?: unknown }>;
}
...
type RowStatus = 'ready' | 'uploading' | 'scheduled' | 'failed';
...
interface QueueRow {
    ...
    account: string;
    deviceUdid: string;
    runAt: string; // '' means "post immediately"
    timeEdited: boolean;
    status: RowStatus;
    message: string;
    scheduleId?: string;
...
const elements = {
    device: element<HTMLSelectElement>('#batch-device'),
    account: element<HTMLSelectElement>('#batch-account'),
    date: element<HTMLInputElement>('#batch-date'),
    slotMorning: element<HTMLInputElement>('#slot-morning'),
    slotAfternoon: element<HTMLInputElement>('#slot-afternoon'),
    slotEvening: element<HTMLInputElement>('#slot-evening'),
    caption: element<HTMLTextAreaElement>('#batch-caption'),
    music: element<HTMLInputElement>('#batch-music'),
    publishConfirm: element<HTMLElement>('#batch-publish-confirm'),
    confirmPublish: element<HTMLInputElement>('#batch-confirm-publish'),
    applyDefaults: element<HTMLButtonElement>('#apply-defaults'),
    shuffle: element<HTMLButtonElement>('#shuffle-times'),
    dropZone: element<HTMLLabelElement>('#drop-zone'),
    fileInput: element<HTMLInputElement>('#file-input'),
    queue: element<HTMLElement>('#queue'),
    queueEmpty: element<HTMLElement>('#queue-empty'),
    queueSummary: element<HTMLElement>('#queue-summary'),
    scheduleAll: element<HTMLButtonElement>('#schedule-all'),
    scheduledList: element<HTMLElement>('#scheduled-list'),
    refreshScheduled: element<HTMLButtonElement>('#refresh-scheduled'),
};
...
interface Slot { id: 'morning' | 'afternoon' | 'evening'; startMin: number; endMin: number; }
const SLOTS: Slot[] = [
...
const rows: QueueRow[] = [];
const rowElements = new Map<number, RowElements>();
let nextRowId = 1;
let busy = false;
let lastScheduled: ScheduleRow[] = [];
...
function selectedDestination(): 'draft' | 'publish' {
    const checked = document.querySelector<HTMLInputElement>('input[name="batch-destination"]:checked');
    return checked?.value === 'publish' ? 'publish' : 'draft';
}

function deviceByUdid(udid: string): DeviceInfo | undefined {
    return devices.find((device) => device.udid === udid);
}

function fillAccountSelect(select: HTMLSelectElement, udid: string, selected: string): void {
...
function fillDeviceSelect(select: HTMLSelectElement, selected: string): void {
    select.replaceChildren();
    for (const device of devices) {
        select.add(new Option(`${device.name}${device.connected ? '' : ' (offline)'}`, device.udid));
    }
    if (devices.some((device) => device.udid === selected)) select.value = selected;
}

function slotCheckbox(slot: Slot): HTMLInputElement {
    if (slot.id === 'morning') return elements.slotMorning;
    if (slot.id === 'afternoon') return elements.slotAfternoon;
    return elements.slotEvening;
}

function selectedSlots(): Slot[] {
...
function assignTimes(): void {
    ...
    const occupy = (udid: string, time: number) => {
    ...
    for (const schedule of lastScheduled) {
        if (schedule.taskType !== 'post' || schedule.status !== 'active' || !schedule.nextRunAt) continue;
        const when = new Date(schedule.nextRunAt).getTime();
        if (!Number.isNaN(when)) occupy(elements.device.value, when);
    }

    const bucketAt = (index: number): { start: number; end: number } => {
    ...
    for (const row of active) {
        ...
        if (row.timeEdited) {
            ...
            if (!Number.isNaN(edited)) occupy(row.deviceUdid, edited);
            bucketIndex += 1;
            continue;
        }
        let placed = false;
        while (bucketIndex < maxBuckets && !placed) {
            ...
            if (earlier.length) winStart = Math.max(winStart, Math.max(...earlier) + gap);
            if (winStart >= winEnd) continue; // slot can't hold this row on this device — roll on
            let chosen = 0;
            for (let attempt = 0; attempt < 30; attempt += 1) {
                const candidate = Math.round((winStart + Math.random() * (winEnd - winStart)) / 60_000) * 60_000;
                if (!times.some((time) => Math.abs(time - candidate) < gap)) { chosen = candidate; break; }
            }
            if (!chosen) continue;
            occupy(row.deviceUdid, chosen);
            row.runAt = toLocalInput(new Date(chosen));
            const refs = rowElements.get(row.id);
            if (refs) refs.when.value = row.runAt;
            placed = true;
        }
        if (!placed) {
            row.runAt = '';
            const refs = rowElements.get(row.id);
            if (refs) refs.when.value = '';
        }
    }
    renderSummary();
}
...
function renderSummary(): void {
    ...
    const parts: string[] = [];
    if (rows.length) parts.push(`${rows.length} in queue`);
    ...
    if (noSlots && rows.length) {
        ...
        if (days.length) {
            const fmt = (value: string) => new Date(`${value}T00:00`).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' });
...
function applyRowStatus(row: QueueRow): void {
    ...
    for (const control of refs.root.querySelectorAll<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | HTMLButtonElement>('input, select, textarea, button')) {
        if (control === refs.schedule) continue;
...
function renumber(): void {
    rows.forEach((row, index) => {
        const refs = rowElements.get(row.id);
        if (refs) refs.index.textContent = String(index + 1);
    });
}

function moveRow(id: number, delta: number): void {
    ...
    for (const item of rows) {
        ...
        if (refs) elements.queue.append(refs.root);
    }
    renumber();
    assignTimes();
}
...
function buildRow(row: QueueRow): void {
    ...
    video.addEventListener('loadedmetadata', () => {
        if (Number.isFinite(video.duration)) {
            ...
            try { video.currentTime = Math.min(0.1, video.duration / 2); } catch { /* ignore */ }
        }
    });
    ...
    up.type = 'button'; up.className = 'icon-button'; up.textContent = '↑'; up.title = 'Move earlier';
    up.addEventListener('click', () => moveRow(row.id, -1));
    const down = document.createElement('button');
    down.type = 'button'; down.className = 'icon-button'; down.textContent = '↓'; down.title = 'Move later';
    down.addEventListener('click', () => moveRow(row.id, 1));
    const remove = document.createElement('button');
    remove.type = 'button'; remove.className = 'icon-button'; remove.textContent = 'Remove';
...
function applyDefaultsToRows(): void {
    for (const row of rows) {
        if (row.status === 'scheduled' || row.status === 'uploading') continue;
        ...
        const refs = rowElements.get(row.id);
        if (refs) {
            (refs.root.querySelector('.queue-caption') as HTMLTextAreaElement).value = row.caption;
            (refs.root.querySelector('.queue-caption') as HTMLTextAreaElement).dispatchEvent(new Event('input'));
            (refs.root.querySelector('input[type="url"]') as HTMLInputElement).value = row.music;
...
function rowTiming(row: QueueRow): { kind: 'now' } | { kind: 'once'; runAt: string } {
    if (!row.runAt) return { kind: 'now' };
...
async function scheduleRow(row: QueueRow): Promise<void> {
    if (row.status === 'scheduled' || row.status === 'uploading') return;
    ...
    try {
        ...
        const data = await response.json() as ScheduleRow & { error?: string };
        if (!response.ok) throw new Error(data.error ?? `Request failed (${response.status})`);
...
async function scheduleAll(): Promise<void> {
    ...
    busy = false;
    renderSummary();
    for (const row of rows) applyRowStatus(row);
    await loadScheduled();
}

function timingDescription(timing: ScheduleRow['timing']): string {
    if (timing.kind === 'once') return `Once · ${formatWhen(timing.runAt ?? '')}`;
    if (timing.kind === 'daily') return `Daily · ${timing.localTime ?? '—'}`;
...
function musicHost(url: string | undefined): string {
    if (!url) return '';
    try { return new URL(url).pathname.replace(/^\/+/, '').slice(0, 40) || 'sound'; } catch { return 'sound'; }
}

async function actOnSchedule(id: string, action: 'pause' | 'resume' | 'cancel'): Promise<void> {
    try {
...
function renderScheduled(schedules: ScheduleRow[]): void {
    ...
    elements.scheduledList.replaceChildren(...posts.map((schedule) => {
        ...
        if (schedule.payload.caption) bits.push(`“${schedule.payload.caption.slice(0, 60)}”`);
        if (schedule.payload.musicUrl) bits.push(`♪ ${musicHost(schedule.payload.musicUrl)}`);
        if (schedule.nextRunAt) bits.push(`next ${new Date(schedule.nextRunAt).toLocaleString()}`);
...
async function loadScheduled(): Promise<void> {
    ...
    if (!udid) return;
    try {
        const data = await jsonRequest<{ schedules: ScheduleRow[] }>(`/api/schedules?deviceUdid=${encodeURIComponent(udid)}`);
...
function updatePublishConfirm(): void {
...
async function init(): Promise<void> {
    ...
    elements.fileInput.addEventListener('change', () => {
        if (elements.fileInput.files) addFiles(elements.fileInput.files);
    ...
    for (const radio of document.querySelectorAll<HTMLInputElement>('input[name="batch-destination"]')) {
    ...
    try {
        const raw = await jsonRequest<ApiDevice[]>('/api/devices');
        devices = raw
            ...
            .map((device) => ({
                ...
                accounts: Object.values(device.pluginData ?? {}).flatMap((value) => (
                    Array.isArray(value?.accounts) ? value!.accounts.filter((entry): entry is string => typeof entry === 'string') : []
                )),
            }));
    } catch (error) {
    ...
    if (storedDestination === 'publish') {
        const publish = document.querySelector<HTMLInputElement>('input[name="batch-destination"][value="publish"]');
        if (publish) publish.checked = true;
    }
